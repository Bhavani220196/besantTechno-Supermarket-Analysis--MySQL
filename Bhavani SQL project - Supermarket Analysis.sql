...
select Salesman_ID,S_Name from SalesmanDetails where Salesman_ID = any
(select SD_ID from OrderDetails where Purchase_Amt>200);						# ANY 
...
select s.S_Name,c.Customer_Name,c.City,o.Order_No,o.Purchase_Amt from SalesmanDetails s
join CustomerDetails c on (s.Salesman_ID=c.SD_ID)
join OrderDetails o  on (c.Customer_ID=o.CD_ID) group by o.Order_No;
...
Delimiter $$
create function Customer(grade int)
returns varchar(20)
Deterministic
Begin
	declare grade_points varchar(20);
    if Grade >400 then
		set grade_points= (Grade + 100);
	elseif Grade <400 and Grade>=200 then
		set grade_points= (Grade + 50);
	elseif Grade<200 and Grade >=100 then
		set grade_points =(Grade + 25);
	end if;
    return (grade_points);
end $$
Delimiter ;

drop function Customer;
show function status where db= 'supermarket_details';
select Customer_ID, Customer_Name, Grade,Customer(grade) from CustomerDetails;
